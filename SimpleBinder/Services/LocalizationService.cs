...
public partial class SimpleBinder
{
    ...
    private void ChangeLanguage(string lang)
    {
        ...
        if (binderIsEnabled) statusButton.Text = statusButton_Turn_Off;//фикс того, что при смене языка, если включен, отображался текст после смены языка как-будто выключен
